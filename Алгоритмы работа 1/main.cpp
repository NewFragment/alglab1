...
list* input(int b)
{
	...
	output(A, b);
	cout << "-------------------------------------------------------------" << endl << endl ;
	return A; //с помощью этой функции переводим массив в список
}
//--------------------------------------------------------------------------------------
...
list* check(list*A, list*B, list*C, list*D)
{
	...
	for (int i = 0; i < 16; i++) //сохраняем результат в массив
	{
		if (u1[i] == 1) //берём наш универсум и если мы наткнулись на единицу
		{
			spisok = check_02(spisok, count, i); //то записываем в массив порядковый номер
